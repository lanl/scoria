...
#define CHECK_IMPL(ind1, ind2, IDX)                                            \
  double *res =                                                                \
      read_data(data, N, ind1, ind2, time_read, num_threads, intrinsics);      \
  ...
  write_data(data, N, input, ind1, ind2, time_write, num_threads, intrinsics); \
                                                                               \
  start_idx[0] = 0;                                                            \
...
bool run_test_suite(size_t N, size_t cluster_size, double alias_fraction,
                    size_t num_threads, i_type intrinsics, uint64_t *time_read,
                    uint64_t *time_write) {
...
void benchmark(size_t N, size_t cluster_size, double alias_fraction,
  ...
  for (size_t i = 0; i < num_runs; ++i) {
    ...
    all_pass &= run_test_suite(N, cluster_size, alias_fraction, num_threads,
                               intrinsics, time_read, time_write);
